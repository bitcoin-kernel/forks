...
export class PeerSource {
  ...
  // anyPeer=true (forks extension): peers only advertise their newest 100 blocks
  // but serve anything in their cache, so if nobody advertises the hash, ask a
  // free peer anyway — a miss is one cheap `noblock` round-trip.
  requestBlock(hash, height = null, timeoutMs = 20000, anyPeer = false) {
    ...
    if (!st && anyPeer) {
      for (const [pid, s] of this.state) {
        if (!s.inboundReq && this.core.channel(pid)?.readyState === 'open') { id = pid; st = s; break; }
      }
    }
